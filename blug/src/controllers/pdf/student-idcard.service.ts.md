...
export class StudentIDCardService {
  ...
  constructor() {
    this.templateDir = "src/controllers/pdf/html"
  }
...
private async getFrontHtml(
  ...
  const template = await this.loadTemplate("front.html");
...
private async getBackHtml(
  ...
  // Nothing to render → skip back page entirely
  if (!hasQr && !hasSignatureSection) {
    return null;
  }

  const template = await this.loadTemplate("back.html");
 ...
 */
  private async loadTemplate(fileName: string): Promise<string> {
    return fs.readFile(
      path.join(process.cwd(), this.templateDir, fileName),
      "utf-8"
    );
  }
...
//     await fs.access(path.dirname(templatePath), fs.constants.F_OK);
//   } catch {
...
//   private async combinePdfPages(...buffers: Buffer[]): Promise<Buffer> {
//   const pdf = await PDFDocument.create();

//   for (const buffer of buffers) {
//     const doc = await PDFDocument.load(buffer);
//     const [page] = await pdf.copyPages(doc, [0]);
//     pdf.addPage(page);
//   }
...
// }
  ...
  //   const front = await PDFDocument.load(frontBuffer);
  //   const back = await PDFDocument.load(backBuffer);

  //   const [frontPage] = await pdf.copyPages(front, [0]);
  //   const [backPage] = await pdf.copyPages(back, [0]);
