      ...
      div(style="display:flex; padding:25px; position:relative; z-index:1;")
        ...
        div(style="flex:1; display:flex; flex-direction:column; justify-content:center; gap:8px;")
          ...
          div
            ...
            if gender
              div(style="display:flex;")
                div.info-label Gender:
                div.info-value= gender.charAt(0).toUpperCase() + gender.slice(1)
            if bloodGroup
              div(style="display:flex;")
                div.info-label Blood Group:
                div.info-value= bloodGroup

      //- Footer
      .footer
        div VALID: #{formatDate(dateIssue)} - #{formatDate(dateDue)}
