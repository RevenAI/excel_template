...
export interface IStudentRecord {
  //references
  studentId: { //populated studentId objectId
    _id: string;
    fullName: string;
    studentId: string; //auto generated ID-Card ID - unique per student
  };
  classSection: {
    _id: string;
    sectionName: string;
...
export function buildStudentSheet(
  ...
  const sheet = workbook.addWorksheet(student.studentId.fullName, {
  ...
  [
    ...
    ["Class", student.classSection.sectionName],
    ["Student ID", student.studentId.studentId],
  ].forEach(([label, value]) => {
